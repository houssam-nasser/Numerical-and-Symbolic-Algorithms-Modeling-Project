...
int main()
{
    ...
    // Measure time for Strassen's inversion
    clock_t start_time = clock(); // Start timing

    strassen_inversion(A, A_inv, n); // Perform Strassen's inversion

    clock_t end_time = clock(); // End timing

    // Calculate and display elapsed time
    double elapsed_time = (double)(end_time - start_time) / CLOCKS_PER_SEC;
    printf("Computation time for Strassen's inversion: %.6f seconds\n", elapsed_time);
    ...
    strassen_inversion(A, A_inv, n);

    printSqMatrix("\nA", A, n);
    printSqMatrix("\nInverse of A", A_inv, n);
